...
Three additional developer-describing properties on the `profiles` table — `headline`, `skills`, and `availability` — plus public exposure of `resume_url`, so that consumers (the project modal credits and the developer profile popup) can render richer developer information without extra requests. Developers can self-edit these fields.
