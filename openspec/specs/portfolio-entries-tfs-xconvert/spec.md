# portfolio-entries-tfs-xconvert Specification

## Purpose

The published-portfolio-entry contract for the two macOS apps added to keisar.club — **ThreeFingerSwitcher** (`WEB-THREEFINGERSWITCHER`) and **Xconvert** (`WEB-XCONVERT`): the required bilingual (EN/HE) fields, media in the `products-image` bucket, link cards, ordering/visibility metadata, and the publish gate — such that each entry renders completely and correctly in the code carousel and project modal.
