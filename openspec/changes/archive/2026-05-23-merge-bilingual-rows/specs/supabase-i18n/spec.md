...
**Reason**: Replaced by JSONB `{en, he}` columns on a single row per SKU. The `lang` column no longer exists; `(SKU, lang)` uniqueness becomes `(SKU)` uniqueness. See the new requirement "Projects table bilingual JSONB" in this same spec.

**Migration**: Existing EN+HE row pairs are merged in the same Supabase migration that introduces the JSONB columns. EN values land in `value.en`, HE values in `value.he`. Where the two rows disagree on a non-translatable field, the EN row's value is kept (audit logged via `RAISE NOTICE`).
